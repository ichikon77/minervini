...
BEAR_WORDS = ["売りや", "売るべき", "売った方", "あかんで", "あかんのや", "持ちたくない",
              ...
              "二番煎じ", "大きな損失を出している", "損失ばかり",
              # 「そろそろ成果を出したほうがいい」= 皮肉の批判。
              # bullの「いいと思うで」と重なるが長い語が勝つルールでbearが優先される
              "成果を出したほうがいい", "成果を出した方がいい",
              "成果を出したほうがええ", "成果を出した方がええ",
              "結果を出したほうがいい", "結果を出した方がいい",
              "結果を出したほうがええ", "結果を出した方がええ",
              "違うものを探したい", "他を探したい",
...
ALT_PATTERNS = [r'を買うといい', r'を買うとええ',
                ...
                # 「それなら(わいは)XXを買う」= 質問銘柄を拒否して別銘柄に乗り換え
                # 例:「Coupangは好きじゃない。それならわいはMercadoLibreを買う」
                r"(?:それなら|そんなら|それやったら)(?:わいは)?\s*[A-Za-z][A-Za-z\s\.&’\']{1,30}\s*を買う",
                r"わいなら\s*[A-Za-z][A-Za-z\s\.&’\']{1,30}\s*を買う",
                # 「一番最初に買いたいのはXXや」= 優先順位で別銘柄を挙げる
                r'(?:一番)?最初に買いたいのは\s*[A-Za-z]',
                r'真っ先に買いたいのは\s*[A-Za-z]']
