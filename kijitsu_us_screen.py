# -*- coding: utf-8 -*-
"""
信用期日スクリーナー US版（需給サイクル・下落銘柄スクリーナー） → kijitsu_us.html → GitHub Pages公開
...
<title>信用期日スクリーナー US版 - {updated_date}</title>
<style>
...
<body>
  ...
  <nav class="nav">
    ...
    <a href="kijitsu_us.html" class="active" style="border-color:#db2777">信用期日(US)</a>
    <a href="kasetsu.html" style="border-color:#94a3b8">仮説検証</a>
  </nav>
  <h1>信用期日スクリーナー US版 — 下落銘柄の経過日数×需給</h1>
...
def main():
    ...
    log("信用期日スクリーナー US版 開始" + ("（テスト: 50銘柄）" if test else ""))
