...
バックテスト（2026-08-13、時価総額500億以上1,334銘柄・10年・5,680エピソード）:
  ...
  → 色分けは「下落初期(〜149日) / 需給最悪期(150〜239日) / 需給改善期(240〜389日) / 長期低迷(390日〜)」
...
<body>
  ...
  <div class="legend">
    <span class="chip zone-early">下落初期（〜149日）</span>
    <span class="chip zone-tunnel">需給最悪期（150〜239日）</span>
    <span class="chip zone-passed">需給改善期（240〜389日）</span>
    <span class="chip zone-long">長期低迷（390日〜）</span>
...
# -----------------------------------------
# 検証マトリクス + 打診・ナンピンの目安（フォワード検証セクション）
# -----------------------------------------
# 直近の検証日。全数フォワード検証（時価総額1,000億以上+営業黒字938銘柄・過去10年日足）を
# 半年〜1年に1回程度、最新データで回し直してこの日付と表を更新する想定。
# 再検証時はこの日付・表・下の打診プランの数値をまとめて書き換えること（一度限りの検証ではない）。
VERIFICATION_DATE = "2026-08-15"

VERIFICATION_MATRIX_ROWS = [
    # (経過日数ラベル, 平均DD, 1ヶ月, 2ヶ月, 3ヶ月, 4ヶ月, 5ヶ月, 6ヶ月, N(2ヶ月), マーク)
    ("0-29日", "-20.4%", (59.0, 2.4), (60.2, 5.0), (62.8, 8.4), (62.1, 9.6), (60.5, 10.6), (62.0, 13.1), "13,295", ""),
    ("30-59日", "-22.3%", (59.2, 2.4), (61.5, 5.1), (61.3, 6.5), (59.9, 7.0), (61.1, 8.9), (59.6, 9.8), "41,017", ""),
    ("60-89日", "-23.1%", (59.8, 2.1), (59.1, 3.6), (59.5, 4.4), (59.0, 5.5), (57.7, 6.2), (57.8, 7.0), "48,734", ""),
    ("90-119日", "-24.1%", (57.2, 1.6), (57.1, 2.7), (57.9, 3.9), (56.0, 4.3), (55.7, 5.1), (56.9, 6.1), "50,283", ""),
    ("120-149日", "-25.1%", (55.6, 1.3), (57.9, 2.9), (56.4, 3.4), (55.9, 3.9), (56.4, 5.1), (57.6, 5.8), "46,799", ""),
    ("150-179日", "-26.0%", (57.1, 1.6), (54.7, 2.0), (54.9, 2.6), (55.4, 3.7), (56.9, 4.7), (56.5, 5.5), "42,942", "←期日直前"),
    ("180-209日", "-26.9%", (51.5, 0.6), (52.0, 1.1), (53.1, 2.0), (55.3, 3.1), (55.7, 4.0), (58.1, 5.8), "40,172", "←期日通過"),
    ("210-239日", "-28.1%", (53.0, 0.7), (53.6, 1.5), (55.1, 2.5), (57.2, 3.6), (58.6, 5.2), (62.4, 7.7), "36,723", ""),
    ("240-269日", "-29.6%", (55.5, 1.2), (57.1, 2.2), (58.3, 3.4), (59.1, 4.9), (63.2, 7.4), (65.6, 9.5), "32,875", ""),
    ("270-299日", "-31.1%", (56.4, 1.4), (57.0, 2.4), (57.9, 3.8), (62.0, 6.4), (65.0, 8.3), (64.6, 9.6), "27,598", ""),
    ("300-329日", "-32.8%", (56.0, 1.4), (56.1, 2.5), (60.8, 5.1), (63.3, 7.2), (64.8, 8.5), (64.6, 9.3), "22,976", ""),
    ("330-359日", "-34.5%", (57.5, 1.7), (61.7, 4.4), (63.9, 6.4), (66.4, 8.0), (65.6, 9.0), (63.5, 9.2), "17,805", "←1年前後"),
    ("360-389日", "-36.2%", (60.7, 2.6), (63.6, 4.4), (64.1, 5.9), (65.1, 7.0), (63.7, 7.7), (62.3, 8.8), "14,208", ""),
    ("390-419日", "-37.7%", (59.2, 2.0), (58.8, 3.5), (59.8, 4.9), (62.2, 6.4), (61.3, 7.4), (59.4, 7.9), "11,096", ""),
    ("420-449日", "-38.9%", (54.8, 1.6), (56.4, 3.0), (60.7, 4.7), (61.4, 5.9), (58.9, 6.3), (59.0, 8.2), "8,943", ""),
    ("450-479日", "-40.4%", (59.7, 2.5), (63.8, 4.8), (61.9, 6.0), (59.9, 6.8), (61.3, 8.4), (58.0, 8.2), "6,652", ""),
    ("480-509日", "-42.1%", (61.0, 2.5), (60.3, 3.7), (57.5, 4.1), (60.9, 6.7), (59.8, 7.1), (57.9, 8.0), "5,267", ""),
    ("510-539日", "-43.6%", (55.1, 1.6), (52.7, 1.7), (57.3, 4.1), (56.3, 4.3), (58.0, 5.9), (57.7, 6.2), "4,266", ""),
    ("540-569日", "-45.4%", (50.7, 0.4), (55.8, 2.9), (58.4, 3.7), (62.1, 5.7), (64.3, 6.5), (61.7, 6.7), "3,161", ""),
    ("570-599日", "-46.5%", (56.8, 2.4), (57.6, 3.4), (62.7, 5.5), (65.1, 6.7), (65.9, 7.1), (61.1, 6.3), "2,695", ""),
    ("600-629日", "-47.2%", (52.6, 0.8), (58.5, 2.7), (63.2, 4.3), (61.3, 4.9), (59.1, 4.7), (57.1, 4.5), "2,207", ""),
    ("630-659日", "-49.7%", (55.1, 1.1), (62.3, 3.4), (61.1, 4.9), (62.3, 5.8), (59.7, 6.8), (60.1, 7.7), "1,765", ""),
    ("660-689日", "-52.4%", (59.2, 1.9), (58.4, 4.2), (62.2, 6.1), (63.4, 9.0), (63.7, 9.7), (63.6, 8.8), "1,356", ""),
    ("690-719日", "-55.2%", (54.2, 1.7), (60.2, 4.9), (64.2, 7.7), (66.2, 9.4), (66.2, 8.6), (70.5, 10.0), "1,099", ""),
    ("720日以上", "-67.5%", (57.6, 1.4), (58.5, 3.6), (59.9, 5.2), (61.9, 6.2), (63.2, 7.9), (63.1, -1.3), "4,413", ""),
]


def _matrix_cell(pair):
...
def build_verification_section():
    """全数フォワード検証マトリクス + 打診・ナンピンの目安。
    kijitsu_screen.pyのユニバース・基準高値ロジックと同一条件で回した検証結果。
    半年〜1年に1回、最新データで再検証してVERIFICATION_DATEと表・下記プランを更新する運用。"""
    rows_html = []
    for label, dd, m1, m2, m3, m4, m5, m6, n, mark in VERIFICATION_MATRIX_ROWS:
        ...
        rows_html.append(
            ...
            f'{_matrix_cell(m1)}{_matrix_cell(m2)}{_matrix_cell(m3)}{_matrix_cell(m4)}{_matrix_cell(m5)}{_matrix_cell(m6)}'
            f'<td style="color:#475569; font-size:0.72rem">{n}</td></tr>')

    return f"""
  <div class="evidence">
    <b>信用期日サイクル検証マトリクス — 日本株・全938銘柄（信用期日スクリーナーと同一条件・検証日 {VERIFICATION_DATE}）:</b><br>
    対象=時価総額1,000億円以上+直近四半期営業黒字の全938銘柄・過去10年日足・サンプリングなし全数（2ヶ月列で総観測488,347件）。
    観測日=<b>5MA&lt;200MAの下落区間内</b>で<b>基準高値から-15%以上</b>の営業日（このスクリーナーの表示条件と完全一致）。
    平均DD=そのバケツの基準高値からの平均下落率（買える安さの目安）。各セル=その時点で買った場合のNヶ月後の<b>勝率%（太字）と平均リターン%</b>（絶対リターン・配当除く）。
...
{"".join(f"        {r}" + chr(10) for r in rows_html)}      </tbody>
    ...
    <br>
    <b>読みどころ:</b> 谷=<b>180-209日（期日通過直後）</b>で1ヶ月51.5%・2ヶ月52.0%が全表の最低。期日直前（150-179日）はまだ54-57%あるので、
    「期日で需給が軽くなる」のではなく「期日通過の直後に最後の投げが出る」という時間構造。
    本命の買い場は<b>240日以降×4〜6ヶ月保有</b>（勝率59-66%、ピークは330-359日×4ヶ月の66.4%）。
    平均DDは経過とともに単調に深化（30日=-20% → 期日通過=-27% → 1年=-35% → 2年=-68%）し、330-419日はDD-35%前後=配当利回りが1.5倍化した水準で勝率も最高という、
    バリューとタイミングの交点になっている。720日以上×6ヶ月保有は勝率63%でも平均リターン-1.3%と、
    たまに来る構造破綻級の大負けが平均を破壊するゾーンなので高利回りに釣られない方がよい。690-719日×6ヶ月の70.5%はN=1,099と薄く生存者バイアス濃厚のため過信しない。
    <br><br>
    <span style="color:#64748b; font-size:0.74rem">
    この検証は{VERIFICATION_DATE}時点のデータに基づく1回分のスナップショットで、市場構造は時間とともに変化する。
    <b>半年〜1年に1回を目安に、最新の10年日足データで同じ条件でフォワード検証を回し直し、この表・検証日・下記の打診プランの数値を更新する運用</b>としている
    （固定の結論ではなく、定期的にアップデートされる前提のセクション）。
    </span>
  </div>
  <div class="evidence">
    <b>打診・ナンピンの目安（{VERIFICATION_DATE}時点の検証結果より、上記マトリクスに基づく）:</b><br>
    ・<b>1回目の打診</b>: 経過日数<b>240日以降</b>で打診買い（この時点で平均DDは既に-29.6%前後、典型的な銘柄はここからさらに-5%程度しか深掘りしない）<br>
    ・<b>2回目（ナンピン）</b>: そこから<b>-10%下</b>（DDでいうと-37%前後=330-419日帯の最良ゾーンに相当する水準）に置く<br>
    ・<b>想定保有期間</b>: 4〜6ヶ月（この保有期間で勝率59-66%、ピークは330-359日×4ヶ月の66.4%）<br>
    ・<b>撤退条件</b>: 510日を超えても浮上しない場合は構造問題（減配・業績悪化等）を疑う。720日超×6ヶ月保有は勝率63%でも平均リターン-1.3%（たまに来る大負けが平均を破壊するゾーン）で、高利回りに釣られて長居しない<br>
    ・<b>下振れリスクの注記</b>: 「平均的にはあと5%」は分布の中心の話で、240-269日で打診した場合の3ヶ月後リターンは下位25%が-5.6%、<b>15.3%の確率で-10%超・5%の確率で-20%超</b>の下落を食らう（6ヶ月保有でも-10%超が17%残る）。1回のナンピンでは吸収できない下振れが一定割合あることは前提にする<br>
    ・配当がある銘柄はDD-30〜35%水準で利回りが平常時の1.4〜1.5倍に上振れるため、上記の勝率・リターンに加えて配当分も上乗せできる<br>
    ・このプランも上のマトリクス同様、次回の再検証（半年〜1年後の目安）で数値が更新されうる暫定的な目安<br>
    ・USの同種検証は別ページで管理する方針（このページには混ぜない）
  </div>"""
...
def generate_html(rows, weeks):
    ...
    html = HTML_TEMPLATE.format(
        ...
        rows="\n".join(trs),
        verification_section=build_verification_section(),
    )
