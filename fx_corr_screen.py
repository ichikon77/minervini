...
def calc_correlations(codes, names, caps, fx_close, nikkei_close):
    ...
    log(f"  日経平均自体のUSD/JPY相関: 1年={nikkei_corr_1y} 3年={nikkei_corr_3y}")
    ...
    for ci in range(0, len(codes), CHUNK):
        ...
        for c in chunk:
            ...
            if corr_1y is None and corr_3y is None:
                continue
            results.append({
                "code": c,
                "name": names.get(c, c),
    ...
    return results, nikkei_corr_1y, nikkei_corr_3y
...
<body>
  ...
  <p class="subtitle">最終更新: {updated} | 対象: 時価総額{min_mcap}億円以上・直近四半期の営業黒字（{n_total}銘柄で算出） | 相関期間: 1年・3年 | 参考: 日経平均自体のUSD/JPY相関 1年{nikkei_corr_1y}・3年{nikkei_corr_3y}</p>
  <div class="evidence">
    ...
    ・1年相関=直近の市場構造を反映、3年相関=より安定した傾向。両方が同じ方向を向いている銘柄ほど傾向が安定している目安。<br>
    ...
    <div class="table-col">
      ...
      <table>
        <thead><tr>
          ...
          <th>単純相関<br>(1年)</th><th>単純相関<br>(3年)</th>
          <th>超過相関<br>(1年)</th><th>超過相関<br>(3年)</th>
        </tr></thead>
        <tbody>
{rows_weak}
        </tbody>
...
{rows_strong}
  ...
  <p class="note">
    ・<b>ランキングのソート基準</b> = 対日経平均・超過相関の1年値を優先（市場全体の為替感応度を除いた、その銘柄固有の為替感応度でランキング）。
    超過相関(1年)が無い銘柄は超過相関(3年)、それも無ければ単純相関(1年)で代替。単純相関は参考として併記。<br>
    ・<b>データ不足銘柄</b> = 新規上場等で1年/3年分の日次データが揃わない銘柄は相関が算出できないため対象外。<br>
...
def build_row(rank, r):
    return (f'          <tr><td class="rank">{rank}</td>'
            f'<td>{r["code"]} {r["name"]}</td>'
            f'<td>{r["mcap"]:,.0f}</td>'
            f'<td>{r["price"]:,.0f}</td>'
            f'<td class="corr">{_fmt_corr(r["corr_1y"])}</td>'
            f'<td class="corr">{_fmt_corr(r["corr_3y"])}</td>'
            f'<td class="corr">{_fmt_corr(r["excess_corr_1y"])}</td>'
            f'<td class="corr">{_fmt_corr(r["excess_corr_3y"])}</td></tr>')


def generate_html(results, nikkei_corr_1y, nikkei_corr_3y):
    # ソート基準: 超過相関(対日経平均)の1年値を優先。市場全体の為替感応度を除いた
    # 銘柄固有の為替感応度でランキングするため、単純相関ではなくこちらを主指標にする。
    # 超過相関(1年)が無ければ超過相関(3年)、それも無ければ単純相関(1年)で代替。
    def sort_key(r):
        if r["excess_corr_1y"] is not None:
            return r["excess_corr_1y"]
        if r["excess_corr_3y"] is not None:
            return r["excess_corr_3y"]
        return r["corr_1y"] if r["corr_1y"] is not None else 0

    valid = [r for r in results if r["excess_corr_1y"] is not None or r["excess_corr_3y"] is not None]
    ...
    rows_weak = "\n".join(build_row(i + 1, r) for i, r in enumerate(top_weak))
    rows_strong = "\n".join(build_row(i + 1, r) for i, r in enumerate(top_strong))
    ...
    html = HTML_TEMPLATE.format(
        ...
        n_total=len(valid),
        top_n=TOP_N,
        nikkei_corr_1y=_fmt_corr(nikkei_corr_1y).replace('<span class="dim">-</span>', '-'),
        nikkei_corr_3y=_fmt_corr(nikkei_corr_3y).replace('<span class="dim">-</span>', '-'),
        rows_weak=rows_weak,
...
def main():
    ...
    results, nikkei_corr_1y, nikkei_corr_3y = calc_correlations(codes, names, caps, fx_close, nikkei_close)

    if not results:
        log("該当銘柄なし。HTMLは生成しません")
        return

    generate_html(results, nikkei_corr_1y, nikkei_corr_3y)
